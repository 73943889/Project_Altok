...
export default function AdminDashboard() {
  ...
  const handleStatusChange = async (id: string, newStatus: TransactionStatus) => {
    ...
    try {
      ...
      // 2. Extraer la operación actual para enviar la notificación por correo
      ...
      if (currentOp && currentOp.email && currentOp.email !== "Sin correo") {
        try {
          ...
          if (!emailResponse.ok) {
            console.error("❌ Error al despachar el correo:", emailResult.error);
          } else {
            console.log("✅ Correo enviado correctamente. ID:", emailResult.resendId);
          }
        } catch (emailErr) {
      ...
      // 3. Refrescar el estado de forma óptima en la interfaz
      setOperations((prev) => {
        if (!Array.isArray(prev)) return [];
        return prev.map((op) => (op.id === id ? { ...op, status: newStatus } : op));
      });
  ...
  const stats = useMemo(() => {
    ...
    // Considerar todas las variantes posibles de completado (COMPLETADO, COMPLETADA, COMPLETED)
    ...
    // Volumen exclusivo en EUR: Suma únicamente cuando la moneda de envío sea EUR
    const totalEur = completedOps
      .filter((o) => o.send_currency === "EUR")
      .reduce((acc, curr) => acc + curr.send_amount, 0);

    // Volumen exclusivo en PEN: Suma únicamente cuando la moneda de recepción sea PEN (o el envío sea PEN)
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `ValoraTransfer_Reporte_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-12">
      {/* Menú de Navegación Superior del Backoffice original */}
      <AdminNavbar />
      ...
      <main className="max-w-7xl mx-auto px-6 space-y-8 pt-6">
        ...
        {/* METRICAS KPI ACTUALIZADAS (5 TARJETAS) */}
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            <select
              ...
              className="bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white outline-none focus:border-emerald-500 transition-colors w-full md:w-auto cursor-pointer"
            >
        ...
        {/* TABLA PRINCIPAL */}
        <div className="bg-slate-900/80 border border-slate-800 rounded-3xl overflow-hidden shadow-xl backdrop-blur-xl min-h-[420px]">
